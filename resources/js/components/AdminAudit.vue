    ...
    <div class="flex flex-col bg-card rounded-xl shadow-sm border border-light overflow-hidden h-full">
        ...
        <div class="flex-1 overflow-y-auto p-4">
            <div class="bg-linear-to-br from-dark to-[#2d3a4a] text-white p-3 rounded-lg flex justify-between items-center mb-3">
                <h3 class="text-xs text-white/70 font-medium">Audited Sales</h3>
            ...
            <div class="overflow-x-auto">
                <table class="w-full border-collapse">
                    ...
                    <tbody>
                        ...
                        <tr v-for="record in filteredAudit" :key="record.id" class="border-b border-light hover:bg-[#f8faf9] transition-colors">
...
const auditData = ref([
    ...
    { id: 3, date: '2026-02-25', time: '02:00 PM', cat: 'Item', item: 'CASTROL ACTIV 4T 1L', det: '1 Qty', amt: 278, staff: 'KENNETH' }
]);

const filteredAudit = computed(() => {
    let data = [...auditData.value];
    if (searchDate.value) {
        data = data.filter(record => record.date === searchDate.value);
    }
