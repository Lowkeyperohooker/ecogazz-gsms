    ...
    <div class="flex flex-col gap-3 h-full overflow-hidden">
        ...
        <div class="bg-card rounded-xl shadow-sm border border-light p-[12px_16px] shrink-0 flex justify-between items-center">
            <div>
                ...
                <p class="text-[0.65rem] text-gray mt-0.5">Manage fuel pricing and correct system meters for all pumps.</p>
            </div>
            <button @click="saveConfigs" :disabled="isSaving || pumps.length === 0" class="px-4 py-2 bg-linear-to-br from-primary to-primary-hover text-white rounded-lg font-bold text-xs flex items-center gap-1.5 transition-transform hover:-translate-y-px shadow-[0_4px_12px_rgba(61,187,145,0.3)] disabled:opacity-50">
        ...
        <div v-else class="flex-1 flex flex-col gap-2.5 min-h-0 overflow-hidden">
            ...
            <div v-if="activePump" class="bg-card rounded-xl shadow-sm border border-light overflow-y-auto flex-1 no-scrollbar animate-[fadeIn_0.3s_ease-out]">
                ...
                <div class="p-3 grid grid-cols-1 lg:grid-cols-2 xl:grid-cols-3 gap-3">
                    <div v-for="config in activePump.fuel_configs" :key="config.id" class="border border-light rounded-xl p-4 relative bg-[#fcfdfd] transition-all hover:border-primary/50 hover:shadow-sm">
                        
                        <div class="flex justify-between items-center mb-4 border-b border-light pb-2">
                            <span class="text-[0.9rem] font-black text-dark uppercase tracking-[0.5px] flex items-center gap-1.5">
                        ...
                        <div class="grid grid-cols-2 gap-3 mb-4">
                            <div class="flex flex-col gap-1.5">
                                <label class="text-[0.6rem] text-gray font-bold uppercase tracking-[0.5px]">Cost Price</label>
                                <div class="relative">
                                    <span class="absolute left-2.5 top-1/2 -translate-y-1/2 text-gray font-bold text-[0.75rem]">₱</span>
                                    <input type="number" step="any" v-model="config.cost_price" class="w-full p-[6px_8px_6px_20px] border-2 border-light rounded-lg text-right font-mono text-[0.8rem] font-bold transition-all focus:outline-none focus:border-warning focus:ring-2 focus:ring-warning/20 bg-white">
                                </div>
                            </div>
                            <div class="flex flex-col gap-1.5">
                                <label class="text-[0.6rem] text-primary font-bold uppercase tracking-[0.5px]">Selling Price</label>
                                <div class="relative">
                                    <span class="absolute left-2.5 top-1/2 -translate-y-1/2 text-primary font-bold text-[0.75rem]">₱</span>
                                    <input type="number" step="any" v-model="config.selling_price" class="w-full p-[6px_8px_6px_20px] border-2 border-primary/30 rounded-lg text-right font-mono text-[0.85rem] font-extrabold text-primary transition-all focus:outline-none focus:border-primary focus:ring-2 focus:ring-primary/20 bg-primary-light/10 shadow-inner">
                                </div>
                            </div>
                        </div>
                        ...
                        <div class="flex flex-col gap-1.5">
                            <label class="text-[0.6rem] text-gray font-bold uppercase tracking-[0.5px] flex justify-between items-end">
                                Current System Meter
                                <span class="text-[0.5rem] font-medium opacity-70 normal-case">(Liters)</span>
                            </label>
                            <input type="number" step="any" v-model="config.current_meter" class="w-full p-2 border-2 border-light rounded-lg text-right font-mono text-[0.9rem] font-bold transition-all focus:outline-none focus:border-blue focus:ring-2 focus:ring-blue/20 text-dark bg-white shadow-inner">
                            <p class="text-[0.55rem] text-gray leading-tight mt-1">Update this if the physical pump drift causes a discrepancy.</p>
                        </div>
