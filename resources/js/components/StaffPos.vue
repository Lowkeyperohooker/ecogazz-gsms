        ...
        <div class="flex gap-1 bg-card p-1 rounded-xl shadow-[0_2px_8px_rgba(0,0,0,0.04)] shrink-0 mb-2">
            <button 
                ...
                :class="[
                    ...
                    activeMode === 'fuel' 
                        ? 'bg-gradient-to-br from-primary to-[#2fa57d] text-white shadow-[0_4px_15px_rgba(61,187,145,0.3)]' 
                        : 'bg-transparent text-gray hover:bg-light hover:text-dark'
                ]"
            >
                <i class="fa-solid fa-gas-pump"></i> Fuel Sales
                <span :class="['px-1.5 py-[1px] rounded-[10px] text-[0.55rem] font-bold', activeMode === 'fuel' ? 'bg-white/25' : 'bg-light text-gray']">
                    {{ fuelCartCount }}
                </span>
            </button>
            <button 
                ...
                :class="[
                    'flex-1 p-[10px_14px] rounded-lg font-bold text-[0.78rem] flex items-center justify-center gap-2 transition-all relative',
                    activeMode === 'prod' 
                        ? 'bg-gradient-to-br from-blue to-[#4a6ae8] text-white shadow-[0_4px_15px_rgba(91,124,250,0.3)]' 
                        : 'bg-transparent text-gray hover:bg-light hover:text-dark'
                ]"
            >
                <i class="fa-solid fa-oil-can"></i> Products
                <span :class="['px-1.5 py-[1px] rounded-[10px] text-[0.55rem] font-bold', activeMode === 'prod' ? 'bg-white/25' : 'bg-light text-gray']">
                    {{ prodCartCount }}
                </span>
            </button>
        </div>
                ...
                <div v-if="activeMode === 'fuel'" class="flex-1 flex flex-col gap-2 overflow-y-auto">
                    ...
                    <div class="grid grid-cols-3 gap-2 shrink-0">
                        <div 
                            ...
                            :class="[
                                'bg-card border-2 rounded-xl p-[14px] cursor-pointer transition-all relative overflow-hidden',
                        ...
                        >
                            <div :class="['w-[34px] h-[34px] rounded-lg flex items-center justify-center text-[0.8rem] font-extrabold text-white mb-2', fuel.bgClass]">
                                {{ fuel.icon }}
                            </div>
                    ...
                    <div v-if="selectedFuel" class="bg-card rounded-xl p-[14px] border border-light shadow-sm shrink-0 animate-[slideUp_0.3s_ease-out]">
                        ...
                        <div class="flex flex-wrap gap-1 mb-2">
                            <button v-for="amt in [100, 200, 300, 500, 1000]" :key="amt" @click="setAmount(amt)" class="px-[10px] py-[5px] border-[1.5px] border-light rounded-full bg-card text-[0.6rem] font-bold text-dark hover:border-primary hover:bg-primary-light hover:text-primary transition-all active:scale-95">
                                ₱{{ amt }}
                            </button>
                        </div>
                        ...
                        <div class="grid grid-cols-2 gap-2 mb-2.5">
                            <div class="flex flex-col">
                                <label class="text-[0.55rem] font-bold text-gray uppercase tracking-[0.5px] mb-[3px]">Amount (₱)</label>
                            ...
                            <div class="flex flex-col">
                                <label class="text-[0.55rem] font-bold text-gray uppercase tracking-[0.5px] mb-[3px]">Liters (L)</label>
                        ...
                        <button @click="addFuelToCart" class="w-full p-[11px] bg-gradient-to-br from-primary to-[#2fa57d] text-white border-none rounded-lg font-bold text-[0.82rem] flex items-center justify-center gap-2 transition-all hover:-translate-y-px shadow-[0_6px_20px_rgba(61,187,145,0.3)] active:translate-y-0">
                ...
                <div v-if="activeMode === 'prod'" class="flex-1 flex flex-col gap-1.5 overflow-hidden">
                    <div class="flex gap-1.5 shrink-0 items-center">
                        <div class="relative flex-1">
                            <i class="fa-solid fa-search absolute left-[11px] top-1/2 -translate-y-1/2 text-gray text-[0.75rem]"></i>
                    ...
                    <div class="flex gap-1 overflow-x-auto shrink-0 pb-0.5 no-scrollbar">
                        <button 
                            ...
                            :class="[
                                ...
                                currentBrand === brand 
                                    ? 'bg-gradient-to-br from-blue to-[#4a6ae8] text-white border-transparent shadow-[0_4px_12px_rgba(91,124,250,0.25)]' 
                    ...
                    <div class="grid grid-cols-2 sm:grid-cols-3 gap-1.5 overflow-y-auto flex-1 p-0.5 content-start min-h-0">
                        <div v-for="(prod, idx) in filteredProducts" :key="idx" class="bg-card border-2 border-light rounded-xl p-2.5 flex flex-col gap-[3px] transition-all hover:border-blue hover:-translate-y-0.5 shadow-sm relative overflow-hidden animate-[fadeIn_0.4s_ease-out]">
                            ...
                            <div :class="['text-[0.5rem] font-medium flex items-center gap-[3px]', prod.s < 10 ? 'text-danger' : 'text-gray']">
                                <i :class="['fa-solid text-[0.4rem]', prod.s < 10 ? 'fa-triangle-exclamation' : 'fa-box']"></i> Stock: {{ prod.s }}
                            </div>
                            <div class="flex gap-1 mt-1.5">
                                <input type="number" v-model="prod.addQty" min="1" class="w-[36px] p-[5px_2px] border-2 border-light rounded-lg text-center font-bold text-[0.7rem] transition-all focus:outline-none focus:border-blue focus:ring-[3px] focus:ring-blue/15">
                                <button @click="addProductToCart(prod)" class="flex-1 p-1.5 bg-gradient-to-br from-blue to-[#4a6ae8] text-white border-none rounded-lg font-bold text-[0.62rem] flex items-center justify-center gap-1 transition-all hover:-translate-y-px shadow-[0_4px_12px_rgba(91,124,250,0.3)] active:translate-y-0">
            ...
            <div class="flex-[0.9] flex flex-col overflow-hidden min-w-0">
                <div class="bg-card rounded-xl p-2.5 border border-light shadow-sm flex-1 flex flex-col overflow-hidden min-h-0">
                    ...
                    <div class="flex-1 overflow-y-auto border border-light rounded-lg min-h-0">
                        <div v-if="cart.length === 0" class="flex flex-col items-center justify-center py-[30px] px-[10px] text-gray h-full">
                            ...
                            <tbody>
                                <tr v-for="item in reversedCart" :key="item.id" class="hover:bg-[#f8faf9] transition-colors border-b border-light">
                                    <td :class="['p-[5px_8px] text-[0.65rem] font-bold whitespace-nowrap border-l-4', item.cat === 'Fuel' ? 'border-primary' : 'border-blue']">
                                        <span :class="['inline-flex px-[5px] py-[1px] rounded-lg text-[0.45rem] font-bold mr-1', item.cat === 'Fuel' ? 'bg-primary-light text-primary' : 'bg-[#eef2ff] text-blue']">
                                            {{ item.cat === 'Fuel' ? '⛽' : '📦' }}
                                        </span>
                                        {{ item.desc }}
                                    </td>
                                    ...
                                    <td class="text-center p-[5px_8px]">
                                        <button @click="removeFromCart(item.id)" class="w-[22px] h-[22px] rounded-md bg-[#fef0f0] text-danger text-[0.6rem] flex items-center justify-center transition-all hover:bg-danger hover:text-white hover:scale-110">
                    ...
                    <div class="grid grid-cols-2 gap-2 my-1.5 shrink-0">
                        <div class="bg-gradient-to-br from-primary-light to-[#d4f2e5] border border-[#b8e5d0] p-[8px_10px] rounded-lg text-center">
                    ...
                    <div class="shrink-0 my-1">
                        ...
                        <div v-show="showDeductions" class="grid grid-cols-3 gap-1 py-1.5 animate-[slideUp_0.3s_ease-out]">
                            <div v-for="(val, key) in deductions" :key="key" class="flex flex-col">
                                <label class="text-[0.48rem] text-gray font-bold uppercase mb-[2px] tracking-[0.3px]">{{ key }}</label>
                    ...
                    <div class="flex gap-1.5 justify-end shrink-0 mt-1">
                        ...
                        <button @click="submitShift" class="p-[8px_18px] rounded-lg font-bold border-none cursor-pointer text-[0.72rem] flex items-center gap-1.5 transition-all bg-gradient-to-br from-primary to-[#2fa57d] text-white hover:-translate-y-px shadow-sm">
                            <i class="fa-solid fa-paper-plane"></i> Submit Shift
                        </button>
                    </div>
                </div>
            </div>
...
// --- FUEL LOGIC ---
const fuels = [
    { key: 'Diesel', name: 'DIESEL', price: 55.80, icon: 'D', bgClass: 'bg-gradient-to-br from-[#f5a623] to-[#e8941a]' },
    { key: 'Premium', name: 'PREMIUM', price: 56.50, icon: 'P', bgClass: 'bg-gradient-to-br from-danger to-[#d32f2f]' },
    { key: 'Regular', name: 'REGULAR', price: 55.80, icon: 'R', bgClass: 'bg-gradient-to-br from-blue to-[#4a6ae8]' }
];
...
const submitShift = () => {
    if (!cart.value.length) return alert('Nothing to submit');
    alert('Shift Submitted Successfully! (API Call will go here)');
    cart.value = [];
    Object.keys(deductions).forEach(k => deductions[k] = 0);
};
</script>
...
<style scoped>
/* Scoped custom scrollbar for inner containers if needed */
